...
export default function LanguageSwitcher() {
  ...
  const others = (["en", "es", "pt"] as LangCode[]).filter((c) => c !== current);
  ...
  return (
    <div ref={boxRef} className="relative inline-block text-left">
      ...
      >
        <img src={LANGS[current].flag} alt={LANGS[current].label} width={20} height={20} className="rounded-sm" />
      ...
      <div
        role="menu"
        className={`absolute right-0 z-50 mt-2 w-full origin-top rounded-md border border-dashed border-green-500/60 bg-black/85 p-2 shadow-[0_8px_24px_rgba(0,0,0,0.45)] transition-all duration-150 ${open ? "scale-y-100 opacity-100" : "pointer-events-none scale-y-75 opacity-0"}`}
      >
        {others.map((code) => (
          <button
            key={code}
          ...
          >
            <img src={LANGS[code].flag} alt={LANGS[code].label} width={18} height={18} className="rounded-sm" />
